<?php
if ( ! class_exists( 'Fact_News_Grid_Posts_Widget' ) ) {
	/**
	 * Adds Fact News Grid Posts Widget.
	 */
	class Fact_News_Grid_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$grid_posts_widget = array(
				...
				'description' => __( 'Retrive Grid Posts Widgets', 'fact-news' ),
			);
			parent::__construct(
				'fact_news_grid_posts_widget',
				__( 'Adore Widget: Grid Posts Widget', 'fact-news' ),
				$grid_posts_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$read_more_button_label = ( ! empty( $instance['read_more_button_label'] ) ) ? $instance['read_more_button_label'] : __( 'Read More', 'fact-news' );
		...
		public function form( $instance ) {
			...
			$read_more_button_label = isset( $instance['read_more_button_label'] ) ? $instance['read_more_button_label'] : __( 'Read More', 'fact-news' );
			$grid_posts_post_count  = isset( $instance['number'] ) ? absint( $instance['number'] ) : 3;
			$grid_posts_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$grid_posts_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'read_more_button_label' ) ); ?>"><?php esc_html_e( 'Read More Button Label:', 'fact-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show (Min:1 | Max:6 ):', 'fact-news' ); ?></label>
