<?php
if ( ! class_exists( 'Fact_News_Posts_Carousel_Widget' ) ) {
	/**
	 * Adds Fact News Posts Carousel Widget.
	 */
	class Fact_News_Posts_Carousel_Widget extends WP_Widget {
		...
		public function __construct() {
			$carousel_widget = array(
				...
				'description' => __( 'Retrive Posts Carousel Widgets', 'fact-news' ),
			);
			parent::__construct(
				'fact_news_posts_carousel_widget',
				__( 'Adore Widget: Posts Carousel Widget', 'fact-news' ),
				$carousel_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			<div class="widget-header">
				...
				if ( ! empty( $carousel_title ) ) {
					echo $args['before_title'] . esc_html( $carousel_title ) . $args['after_title'];
				}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'slides_to_show' ) ); ?>"><?php esc_html_e( 'Slides to show:', 'fact-news' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'slides_to_show' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'slides_to_show' ) ); ?>" class="widefat" style="width:100%;">
					<option value="one-column" <?php selected( 'one-column', $slides_to_show ); ?>><?php esc_html_e( 'One Column', 'fact-news' ); ?></option>
					<option value="two-column" <?php selected( 'two-column', $slides_to_show ); ?>><?php esc_html_e( 'Two Column', 'fact-news' ); ?></option>
					<option value="three-column" <?php selected( 'three-column', $slides_to_show ); ?>><?php esc_html_e( 'Three Column', 'fact-news' ); ?></option>
					<option value="four-column" <?php selected( 'four-column', $slides_to_show ); ?>><?php esc_html_e( 'Four Column', 'fact-news' ); ?></option>
				</select>
			</p>
			<p>
